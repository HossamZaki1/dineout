...
class ConversationService {
  ...
  static const _historyKeyPrefix =
      'conversation_history:'; // key per conversation

  String _keyForUser(String? userId) =>
      '$_storageKeyPrefix${userId ?? 'guest'}';
  ...
  Future<void> deleteConversation(String id, {String? userId}) async {
    ...
    final encoded = json.encode(existing.map((e) => e.toMap()).toList());
    await prefs.setString(key, encoded);
    await prefs.remove(_historyKeyForConversation(id, userId));
  }
  ...
  Future<void> clearAll({String? userId}) async {
    ...
    final prefix = '$_historyKeyPrefix${userId ?? 'guest'}:';
    for (final historyKey in prefs.getKeys().where(
  ...
  Future<ConversationMeta?> getConversationMeta(
  ...
  Future<void> addMessageToConversation(
    ...
    final existing =
        await getConversationHistory(conversationId, userId: userId) ?? [];
  ...
  List<String> _extractRestaurantNames(List<ConversationMessage> messages) {
    ...
    for (final message in messages) {
      if (message.restaurantSuggestions != null &&
          message.restaurantSuggestions!.isNotEmpty) {
  ...
  Future<void> updateConversationWithSuggestions(
    String conversationId, {
    String? userId,
  }) async {
